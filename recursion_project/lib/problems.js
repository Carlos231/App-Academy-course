...
function flatten(data) {
    ...
    // get rid of all arrays and add them to the final array
    // make sure that it is flattened all the way
    data.forEach((elem) => {
        let flattened = flatten(elem);
        allElements.push(...flattened); // spread operator to add values and not the array itself (not nest again)
    });
...
function pathFinder(directories, targetFile) {
    for (let name in directories) {
        ...
        let subPath = pathFinder(directories[name], targetFile)
        if (subPath !== null) {
            return name + subPath;
        }
    }
